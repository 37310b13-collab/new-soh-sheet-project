...
されています。`Dashboard`のC1セルに`W23`のように入力すると（現在年の週Noとして検索）、該当する週の列が
太枠でハイライトされます。VBAの`JumpToSelectedWeek`（要手動設定・詳細は5.6章）を導入して
いれば、複製データではない本物の該当週列が、`実績週`列のすぐ右（固定ペインの直後）に来る
...
- **TTAF倉庫の在庫実績**: 「CSA Report」の` COUNT SHEET SOH`シート（先頭に半角スペースあり）からマクロで取り込み
...
ラベル列のすぐ右に表示する方式にしています。`Dashboard`は`実績週`列の右（I列〜）、
...
に**前週分**の実績が届く運用です。`COUNT SHEET SOH`シートのH1セルの日付はレポートが届いた
