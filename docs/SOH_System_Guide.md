...
`RefreshTTAFStock`・`RefreshShipments`は、CSA Reportファイルを開いた直後に**ブック内の全ピボット
テーブルを自動で更新（RefreshAll）**してからデータを読み込みます。CSA Report内のピボット
（例:「PIVOT SOH TTAF」）は「Stock invoiced to CSA」等の手入力データを集計元にしており、TTAF側が
ファイルを保存する前にピボットの更新を忘れていても、開いた時点でファイル内のデータから
再集計されるため、こちらのマクロ側では常に正しい値を取り込めます（外部データベース等、
ファイル外部に接続しているピボットではないことを確認済みです）。
