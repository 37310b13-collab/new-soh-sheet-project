...
' 実際に使われている中間体が見つかれば、SyncMaterialDetailIntermediates(RefreshBOMの
' 末尾で自動的に呼ばれる)がこのブロックに中間体の内訳行を自動的に追加する
' （AddMaterialを再度実行する必要はない）。
...
' あれば、その2行を「合計使用量(kg)/週」行の直前にペアで挿入する。これにより、
' AddMaterialで追加したばかりの材料(BOM未登録のため内訳行が無いミニブロック)も、
' RefreshBOMを1回実行するだけで既存の材料と同じ見た目になる(AddMaterialを2回実行する
